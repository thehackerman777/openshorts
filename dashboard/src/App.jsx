...
function App() {
  ...
  useEffect(() => {
    // Encrypt DeepSeek Key too for consistency if desired, but user asked specifically about Social integration not saving well.
  ...
  const handleProcess = async (data) => {
    ...
    try {
      let body;
      const headers = { 'X-DeepSeek-Key': apiKey };
      ...
      const res = await fetch(getApiUrl('/api/process'), {
        method: 'POST',
        headers: data.type === 'url' ? headers : { 'X-DeepSeek-Key': apiKey },
        body
      });
          ...
          <div className="flex items-center gap-4">
            ...
            {(!apiKey || !uploadPostKey) && (
              ...
              >
                ...
                {!apiKey && !uploadPostKey
                  ? 'DeepSeek & Upload-Post keys missing'
                  : !apiKey
            ...
            <div className="flex items-center gap-3 text-sm text-amber-200">
              ...
              <div>
                ...
                <span className="text-amber-200/80">
                  {!apiKey && !uploadPostKey
                    ? 'Set your DeepSeek and Upload-Post API keys to use OpenShorts.'
                    : !apiKey
                      ? 'Set your DeepSeek API key to use OpenShorts.'
            ...
            <div className="h-full overflow-y-auto p-8 max-w-2xl mx-auto animate-[fadeIn_0.3s_ease-out]">
              ...
              <KeyInput onKeySet={setApiKey} savedKey={apiKey} />
                ...
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-lg font-semibold">YouTube Publishing</h2>
                ...
                <p className="text-xs text-zinc-500 mb-6 leading-relaxed">
                  Publish videos directly to YouTube <strong>natively</strong> — no third-party services.
                  Uses OAuth 2.0 with the YouTube Data API v3.
                </p>
                <div className="space-y-4">
                  <div className="flex gap-2">
                    ...
                    >
                      <Youtube size={16} />
                      {youtubeConnected ? 'Connected' : 'Connect YouTube'}
                    </button>
                    {youtubeConnected && (
                      <span className="text-xs text-green-400 flex items-center">Authorized</span>
                    )}
                  </div>
                  ...
                  {showYoutubeCodeInput && !youtubeConnected && (
                    <div className="p-3 bg-white/5 border border-white/10 rounded-lg space-y-2">
                      <p className="text-xs text-zinc-400">
                        After authorizing in Google, you'll be redirected to a page that fails to load. 
                        <strong className="text-white"> Copy the full URL from your browser's address bar</strong> and paste it below:
                      </p>
                      <div className="flex gap-2">
                        <input
                          type="text"
                  ...
                  <p className="text-xs text-zinc-500 leading-relaxed">
                    Setup: In <strong className="text-white">Google Cloud Console</strong>, create OAuth Client ID with redirect URI:<br />
                    <code className="text-primary text-[10px] bg-white/5 px-2 py-0.5 rounded">http://localhost:8000/auth/youtube/callback</code>
                    <br />Then save the JSON file to <code className="text-primary text-[10px]">data/youtube_client_secrets.json</code>
                  </p>
                </div>
              </div>
              ...
              <div className="max-w-4xl mx-auto space-y-8">
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  ...
                  <div className="glass-panel p-5 space-y-2">
                    ...
                    <p className="text-xs text-zinc-400 leading-relaxed">
                      Whisper transcribes, DeepSeek AI spots viral beats, FFmpeg cuts each clip and adds a hook overlay.
                    </p>
                  </div>
                ...
                <div className="glass-panel p-6 md:p-8 space-y-5">
                  ...
                  <div className="grid sm:grid-cols-2 gap-3 text-sm">
                    ...
                    <div className="flex items-start gap-2 text-zinc-300">
                      <Check size={16} className="text-emerald-400 shrink-0 mt-0.5" />
                      <span>DeepSeek AI multimodal moment detection</span>
                    </div>
                    <div className="flex items-start gap-2 text-zinc-300">
                      <Check size={16} className="text-emerald-400 shrink-0 mt-0.5" />
      ...
      {showKeyModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowKeyModal(false)}>
          <div className="bg-[#18181b] border border-white/10 rounded-2xl p-6 max-w-md w-full mx-4 space-y-4 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-zinc-400">
              OpenShorts needs both a <strong className="text-zinc-200">DeepSeek</strong> API key and an <strong className="text-zinc-200">Upload-Post</strong> API key. Both have free tiers.
            </p>
            ...
            <div className={`rounded-lg p-4 space-y-2 border ${!apiKey ? 'bg-blue-500/5 border-blue-500/30' : 'bg-white/5 border-white/10 opacity-70'}`}>
              <p className="text-xs font-semibold text-zinc-200 flex items-center gap-2">
                ...
                DeepSeek API Key {apiKey && <span className="text-green-400">— set</span>}
              </p>
              {!apiKey && (
                <>
            ...
            <div className={`rounded-lg p-4 space-y-2 border ${!uploadPostKey ? 'bg-violet-500/5 border-violet-500/30' : 'bg-white/5 border-white/10 opacity-70'}`}>
              ...
              {!uploadPostKey && (
                <>
                  ...
                  <ol className="text-xs text-zinc-400 space-y-1 list-decimal list-inside">
                    <li>Register at <a href="YouTube/login" target="_blank" rel="noopener noreferrer" className="text-violet-400 underline">YouTube</a></li>
                    <li>Connect your TikTok, Instagram, or YouTube accounts</li>
                    <li>Go to <a href="YouTube/api-keys" target="_blank" rel="noopener noreferrer" className="text-violet-400 underline">API Keys</a> and generate one</li>
