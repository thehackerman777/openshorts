...
# Sessions persistence directory
...
async def process_endpoint(
    ...
    env = os.environ.copy()
    env["DEEPSEEK_API_KEY"] = api_key # Override with key from request
...
async def edit_clip(
    ...
    # Determine API Key
    final_api_key = req.api_key or x_gemini_key or os.environ.get("DEEPSEEK_API_KEY")
    
    if not final_api_key:
        raise HTTPException(status_code=400, detail="Missing DeepSeek API Key (Header or Body)")
...
async def generate_effects_config(
    ...
    final_api_key = x_gemini_key or os.environ.get("DEEPSEEK_API_KEY")

    if not final_api_key:
        raise HTTPException(status_code=400, detail="Missing DeepSeek API Key (Header)")
...
async def thumbnail_upload(
    ...
    async def run_background_whisper():
        """Background task - Whisper skipped: CPU incompatible (no AVX)."""
        print(f"ℹ️ [Thumbnail] Whisper disabled on this CPU (no AVX support). Using empty transcript.")
...
async def thumbnail_generate(
    ...
    try:
        ...
        if not thumbnails:
            raise HTTPException(status_code=500, detail="Thumbnail generation failed. Please check your DeepSeek API key has access to image generation (deepseek-chat model).")
...
async def thumbnail_describe(
    ...
    # Build description context from session (game, title, etc.)
    ...
    try:
        ...
        if segments:
            ...
            # No transcript (Whisper disabled on this CPU)
            # Generate a simple description based on title + game context
...
# ============================================================
# YouTube OAuth + Direct Upload (replaces Upload-Post)
# ============================================================
...
async def thumbnail_publish(
    ...
    if not is_authenticated():
        raise HTTPException(status_code=401, detail="YouTube not connected. Go to Settings to connect your YouTube account.")
        ...
        try:
            ...
            for sid, sched in list(SCHEDULED_PUBLISHES.items()):
                ...
                if sched.get("scheduled_at", 0) <= now:
                    ...
                    # Capture values by default arg to avoid closure-in-loop bug
...
async def list_sessions():
    ...
    # Sort: most recently updated first (by filename timestamp)
...
async def saasshorts_analyze(
    ...
    gemini_key = x_gemini_key or os.environ.get("DEEPSEEK_API_KEY")
    if not gemini_key:
