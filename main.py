...
# YOLO/torch are optional — fallback if CPU supports AVX2
# Clip Generator works with GENERAL mode (blurred background) without YOLO
try:
...
# Only loads if CPU supports AVX2 (otherwise falls back to MediaPipe + GENERAL mode)
if _HAS_YOLO:
    try:
...
def detect_person_yolo(frame):
    ...
    Returns None if YOLO is unavailable (old CPU) — caller falls back to GENERAL mode.
    """
    if not _HAS_YOLO or model is None:
        return None
...
def get_viral_clips(transcript_result, video_duration):
    ...
    print(f"🤖  Initializing DeepSeek with model: {model_name}")
    ...
    try:
        response = client.chat.completions.create(
            model=model_name,
            messages=[
                {"role": "system", "content": "You are a senior short-form video editor. Respond ONLY with valid JSON, no markdown."},
        ...
        try:
            if response.usage:
                ...
                # DeepSeek Chat Pricing: Input $0.27/1M, Output $1.10/1M
